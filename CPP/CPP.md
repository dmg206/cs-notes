...
1. 修饰变量，说明该变量不可以被改变；在类中声明变量为const类型， const常量的初始化必须在构造函数初始化列表中初始化
...
3. 修饰成员变量（类变量），修饰成员变量使所有的对象只保存一个该变量，而且不需要生成对象就可以访问该成员。static 成员必须在类外初始化，static const 成员跟static成员一样在类外初始化
...
友元可以是个函数，该函数被称为友元函数；友元也能够是个类，该类被称为友元类。
...
1，构造是从类层次的最根处开始，然后调用成员对象的构造函数。对于成员对象，构造函数的调用次序完全不受构造函数的初始化列表次序决定，而是定义顺序决定。

2，引用，const成员变量，基类构造函数 一定要通过初始化列表来实现。 static类型不是类对象成员，不需要通过初始化列表来初始化
